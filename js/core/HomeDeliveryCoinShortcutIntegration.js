...
function decorate(){
 ...
 cards.forEach((card,index)=>{
  ...
  const fresh=currentRow(r);let full;try{full=fullOperationTimeReductionQuote(fresh);}catch{controls.remove();return;}const balance=Math.max(0,Math.floor(Number(c.coins||0))),initial=Math.min(balance,full.costCoins);controls.innerHTML='';
