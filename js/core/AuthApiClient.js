...
const RECOVERY_FLAG_KEY = "worldproject_password_recovery";
export class AuthApiClient {
 constructor({baseUrl=SUPABASE_URL,publishableKey=SUPABASE_PUBLISHABLE_KEY,localApiBase="/api/orvuno"}={}){this.baseUrl=baseUrl.replace(/\/$/,"");this.publishableKey=publishableKey;this.localApiBase=String(localApiBase||"/api/orvuno").replace(/\/$/,"");this.session=this.loadSession();this.captureSessionFromUrl();}
 loadSession(){try{return JSON.parse(localStorage.getItem(SESSION_KEY)||"null");}catch{return null;}} saveSession(s){this.session=s||null;if(s)localStorage.setItem(SESSION_KEY,JSON.stringify(s));else localStorage.removeItem(SESSION_KEY);}
 captureSessionFromUrl(){const raw=location.hash?.replace(/^#/,"");if(!raw)return null;const p=new URLSearchParams(raw),access_token=p.get("access_token"),refresh_token=p.get("refresh_token");if(!access_token)return null;const type=p.get("type")||null,s={access_token,refresh_token,expires_in:Number(p.get("expires_in")||3600),token_type:p.get("token_type")||"bearer",expires_at:Math.floor(Date.now()/1000)+Number(p.get("expires_in")||3600),type};this.saveSession(s);if(type==="recovery"){try{sessionStorage.setItem(RECOVERY_FLAG_KEY,"1");}catch{}}history.replaceState(null,document.title,location.pathname+location.search);return s;}
 isPasswordRecovery(){try{return this.session?.type==="recovery"||sessionStorage.getItem(RECOVERY_FLAG_KEY)==="1";}catch{return this.session?.type==="recovery";}}
 async preparePasswordRecovery(){if(this.isPasswordRecovery())return true;const q=new URLSearchParams(location.search),code=q.get("code");if(!code)return false;const verifier=(()=>{try{return sessionStorage.getItem("worldproject_pkce_verifier")||localStorage.getItem("worldproject_pkce_verifier")||"";}catch{return "";}})();const payload={auth_code:code};if(verifier)payload.code_verifier=verifier;const r=await fetch(`${this.baseUrl}/auth/v1/token?grant_type=pkce`,{method:"POST",headers:this.headers(),body:JSON.stringify(payload)}),b=await this.parse(r);this.saveSession({...b,type:"recovery"});try{sessionStorage.setItem(RECOVERY_FLAG_KEY,"1");sessionStorage.removeItem("worldproject_pkce_verifier");localStorage.removeItem("worldproject_pkce_verifier");}catch{}q.delete("code");const search=q.toString();history.replaceState(null,document.title,`${location.pathname}${search?`?${search}`:""}${location.hash||""}`);return true;}
 clearPasswordRecovery(){if(this.session?.type==="recovery")this.saveSession({...this.session,type:null});try{sessionStorage.removeItem(RECOVERY_FLAG_KEY);}catch{}}
 headers({auth=false,json=true}={}){const h={apikey:this.publishableKey};if(json)h["Content-Type"]="application/json";if(auth&&this.session?.access_token)h.Authorization=`Bearer ${this.session.access_token}`;return h;}async parse(r){const b=await r.json().catch(()=>({}));if(!r.ok)throw new Error(b.msg||b.message||b.error_description||b.error||`HTTP ${r.status}`);return b;}
 async health(){const r=await fetch(`${this.baseUrl}/auth/v1/health`,{headers:this.headers({json:false})});if(!r.ok)throw new Error("Supabase nicht erreichbar");return{success:true,mode:"supabase"};}
