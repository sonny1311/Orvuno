...
function patchOperationalDialog(){
 ...
 const oldCard=p.renderProductionCard;p.renderProductionCard=function(parent,...args){const result=oldCard.call(this,parent,...args),card=parent.lastElementChild;if(card&&!card.querySelector('[data-orvuno-production-coin-rule]')){const note=document.createElement('div');note.dataset.orvunoProductionCoinRule='1';note.textContent='🪙 Nach dem Start optional beschleunigen: 1 Coin je angefangene 5 Min. Restzeit (60 Min. = 12 Coins).';Object.assign(note.style,{marginTop:'7px',fontSize:'12px',fontWeight:'800',color:'#8a6500'});card.append(note);}return result;};
...
function decorateMachineMarket(dialog,panel){
 ...
 for(const item of market.filter(x=>x.working&&x.ownedInstance)){const name=item.name||item.label||item.id,card=[...section.children].find(el=>el.tagName==='DIV'&&text(el.textContent).includes(name));if(!card)continue;removeOldAcceleration(card);const raw=item.ownedInstance,r=rowFor('equipment',raw,raw.instanceId||raw.id);appendControl(card,r,c,{onDone:async()=>{modules.equipmentMod.processIndustryEquipmentInstallations(c,{now:Date.now()});await modules.equipmentMod.persistIndustryEquipment(c).catch(()=>{});dialog.ensureMachines?.(c);dialog.__worldFocusedSection='machines';if(dialog.overlay?.firstElementChild)dialog.render(dialog.overlay.firstElementChild);}});}
}
