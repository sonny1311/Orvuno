...
const feedback=(text,type='info')=>{
  ...
  if(api?.show)return api.show(text,{type,title:'Store-Kauf'});
  if(api?.feedback)return api.feedback(text,{type,title:'Store-Kauf'});
  console[type==='error'?'error':'log'](text);
};
function requireBridge(){
  const bridge=nativeBridge();
  if(!bridge||typeof bridge.purchase!=='function')throw new Error('Store-Käufe sind auf diesem Gerät nicht verfügbar.');
  return bridge;
}
...
async function verifyReceipt(detail={}){
  ...
  if(!receiptId||!amazonSku||!amazonUserId||!internalSku)throw new Error('Der Store hat unvollständige Kaufdaten zurückgegeben.');
  ...
  try{
    ...
    else feedback('✅ Kauf bestätigt und gutgeschrieben.','success');
    return body;
...
function updateAmazonCatalogUi(){
  document.querySelectorAll('[data-coin-offer],[data-premium-offer]').forEach(button=>{
    ...
    // niemals blockieren. Der verbindliche Preis wird im nativen Kaufdialog angezeigt.
    button.disabled=false;
    button.title='Abrechnung über den Appstore';
    if(product?.price){
      const price=String(product.price);
      if(button.textContent!==price)button.textContent=price;
    }else if(button.dataset.amazonFallbackLabel!=='1'){
      button.textContent='Im Appstore kaufen';
...
function requestProducts(){
  const bridge=requireBridge();
  if(typeof bridge.requestProductData!=='function')throw new Error('Produktdaten können nicht geladen werden.');
...
function refreshCatalogSoon(delay=0){
  ...
  catalogRetryTimer=setTimeout(()=>{
    try{requestProducts();}catch(error){console.warn('Store-Produktdaten werden später erneut geladen',error);}
  },delay);
}
export async function beginAmazonPurchase({internalSku}={}){
  ...
  if(!amazonSku)throw new Error('Dieses Produkt ist im Appstore nicht verfügbar.');

  // Wie bei Hofhain: der Klick geht direkt an die native Store-Brücke. Ein fehlender
  // Produktdaten-Cache darf keinen toten/gesperrten Kaufbutton erzeugen.
  const bridge=requireBridge();
  feedback('Kauf wird geöffnet …','info');
...
function install(){
  ...
  window.addEventListener('orvuno:amazon-iap-error',event=>feedback(event.detail?.message||'Store-Kauffehler','error'));
...
const provider={id:'amazon',label:'Appstore',begin:beginAmazonPurchase,beginCoinPurchase,beginPremiumPurchase,getCatalogDetails:getAmazonCatalogDetails,restorePurchases:restoreAmazonPurchases};
