...
function liveRemainingMs(row,now){const end=timerEnd(row.raw)?.value||0;return Math.max(0,end-now);}
export function operationTimeReductionQuote(row={},hours=1,{now=Date.now()}={}){
 ...
 const remainingMs=liveRemainingMs(row,now);if(!remainingMs)throw new Error('Der Vorgang ist bereits fertig');
...
export function operationTimeReductionQuoteWithCoinBudget(row={},hours='all',coinBudget=1,{now=Date.now()}={}){
 const base=operationTimeReductionQuote(row,hours,{now}),budget=Math.max(1,Math.min(1000000,Math.floor(n(coinBudget,1)))),budgetMs=budget*COIN_TIME_UNIT_MS;
 const reducedMs=Math.min(base.reducedMs,budgetMs),costCoins=coinCostForMs(reducedMs);
 return{...base,coinBudget:budget,reducedMs,costCoins,newRemainingMs:base.remainingMs-reducedMs,full:reducedMs>=base.remainingMs};
}
export function operationTimeReductionQuoteForCoins(row={},coinBudget=1,options={}){return operationTimeReductionQuoteWithCoinBudget(row,'all',coinBudget,options);}
...
export async function reduceOperationTimeWithCoins(company={},row={},hours=1,{coinBudget=null}={}){
 const base=operationTimeReductionQuote(row,hours),budget=coinBudget==null?null:Math.max(1,Math.min(1000000,Math.floor(n(coinBudget,1)))),q=budget==null?base:operationTimeReductionQuoteWithCoinBudget(row,hours,budget),companyId=Number(company.serverCompanyId);if(!Number.isFinite(companyId)||companyId<=0)throw new Error('Server-Betrieb fehlt');
 ...
 try{
  ...
  // 5-Sekunden-Autosave die alte Endzeit nicht wieder ueber den gerade geaenderten Serverzustand schreiben.
  if(sync&&!sync.saving){sync.saving=true;saveLock=true;}
  if(typeof window!=='undefined')window.worldCoinTimeReductionInFlight=true;
  const payload={p_company_id:companyId,p_action_kind:row.kind,p_action_id:String(row.id),p_hours:base.requestedHours};if(budget!=null)payload.p_max_coins=budget;
  const r=await api.rpc('shorten_company_timed_action',payload);
  ...
  window.dispatchEvent(new CustomEvent('world:game-state-dirty',{detail:{reason:'secure-coin-time-reduction',kind:row.kind,id:row.id,costCoins:n(r?.costCoins),reducedMs,requestedCoinBudget:budget,quotedReducedMs:q.reducedMs}}));
...
export function runOperationCoinTimeReductionTest(){
 ...
 const ninety={kind:'production',id:'p90',remainingMs:90*60000,raw:{id:'p90',finishAt:now+90*60000}},ten=operationTimeReductionQuoteForCoins(ninety,10,{now});if(ten.costCoins!==10||ten.reducedMs!==50*60000||ten.newRemainingMs!==40*60000||ten.full)throw new Error('10 Coins müssen bei 90 Minuten exakt 50 Minuten verkürzen');
 const eighteen=operationTimeReductionQuoteForCoins(ninety,18,{now});if(eighteen.costCoins!==18||eighteen.reducedMs!==90*60000||!eighteen.full)throw new Error('18 Coins müssen 90 Minuten vollständig verkürzen');
 ...
 const twoMin={kind:'delivery',id:'d2',remainingMs:2*60000,raw:{id:'d2',arrivalAt:now+2*60000}},one=operationTimeReductionQuoteForCoins(twoMin,1,{now});if(one.costCoins!==1||one.reducedMs!==2*60000||!one.full)throw new Error('Letzte Restzeit unter 5 Minuten muss mit 1 Coin fertig werden');
...
if(typeof window!=='undefined')window.worldOperationCoinTimeReduction={canReduce:canReduceOperation,quote:operationTimeReductionQuote,fullQuote:fullOperationTimeReductionQuote,quoteForCoins:operationTimeReductionQuoteForCoins,quoteWithBudget:operationTimeReductionQuoteWithCoinBudget,costForMs:coinCostForMs,rule:COIN_TIME_RULE_LABEL,reduce:reduceOperationTimeWithCoins,test:runOperationCoinTimeReductionTest};
