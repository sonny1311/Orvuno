...
export async function beginGooglePlayPurchase({internalSku}={}){
  ...
  // PaymentRequest.show() muss direkt im ursprünglichen Tap-Handler laufen.
  // Keine await-/Netzwerkoperation darf davor liegen, sonst verliert Chrome die transient user activation.
  const product=cachedProductFor(internalSku);
  ...
  try{
    ...
    console.error('[ORVUNO Google Play] Kauf fehlgeschlagen',{
      ...
      code:errorCode,
      userActivation:typeof navigator!=='undefined'&&navigator.userActivation?{isActive:navigator.userActivation.isActive,hasBeenActive:navigator.userActivation.hasBeenActive}:null
    });
