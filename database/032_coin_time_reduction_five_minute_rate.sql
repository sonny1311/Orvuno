-- ORVUNO: einheitliche Zeitverkuerzung mit Coins.
-- Regel: 1 Coin je angefangene 5 Minuten tatsaechlich verkuerzter Restzeit.
-- Die Abbuchung und Timer-Aenderung erfolgen atomar auf dem Server.

CREATE OR REPLACE FUNCTION private.orvuno_jsonb_time_ms(p_value jsonb)
RETURNS bigint
LANGUAGE plpgsql
SET search_path TO ''
AS $$
...
REVOKE ALL ON FUNCTION private.orvuno_jsonb_time_ms(jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION private.orvuno_jsonb_time_ms(jsonb) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION private.orvuno_jsonb_time_ms(jsonb) TO service_role;

CREATE OR REPLACE FUNCTION public.shorten_company_timed_action(
  p_company_id bigint,
  p_action_kind text,
  p_action_id text,
...
BEGIN
  ...
  -- Verbindliche ORVUNO-Regel: 1 Coin je angefangene 5 Minuten.
  ...
  -- Dadurch koennen alternative ETA-/Finish-Felder die Verkuerzung nicht wieder rueckgaengig machen.
  FOREACH v_end_key IN ARRAY v_end_keys
  LOOP
  ...
  VALUES(
    ...
    p_action_kind,
    p_action_id,
    format('%s Coin(s) fuer %s Minuten Zeitverkuerzung (1 Coin je angefangene 5 Minuten)', v_cost, ceil(v_reduction_ms / 60000.0))
  );
...
REVOKE ALL ON FUNCTION public.shorten_company_timed_action(bigint,text,text,integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.shorten_company_timed_action(bigint,text,text,integer) FROM anon;
GRANT EXECUTE ON FUNCTION public.shorten_company_timed_action(bigint,text,text,integer) TO authenticated, service_role;
